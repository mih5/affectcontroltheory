runSimulation <- function(sim.B, trainSize, testSize, priors){
  ...
  sim.model.BMA.prior <- bic.glm(x = train.data[,c(-1,-2)], y = train.data[,"response"], maxCol=30, glm.family = gaussian(), prior.param = priors)
  #store identified variables
  var.iden.BMA.prior <- sim.model.BMA.prior$namesx[sim.model.BMA.prior$probne0>50]
  ...
  # BMS WITH PRIOR
  sim.model.BMS.prior <- bms(train.data[,-2], iter=10000, mprior="pip", mprior.size = priors)
  var.iden.BMS.prior <- rownames(coef(sim.model.BMS.prior))[which(coef(sim.model.BMS.prior)[,1]>0.5)]
  ...
  #BMA w prior
  coef.BMA.prior <- sim.model.BMA.prior$postmean
  
  #BMS w/o prior
  coef.BMS <- coef(sim.model.BMS, include.constant=TRUE)[,"Post Mean"]
  
  #BMS w prior
  coef.BMS.prior <- coef(sim.model.BMS.prior, include.constant=TRUE)[,"Post Mean"]
  ...
  return(list("var.iden" = list("truth" = var.model,
                                ...
                                "bma" = var.iden.BMA,
                                "bms" = var.iden.BMS,
                                "bma.prior" = var.iden.BMA.prior,
                                "bms.prior" = var.iden.BMS.prior),
         "coef.est" = list("stepwise" = coef.stepwise,
                           "anova" = coef.anova,
                           "bma" = coef.BMA,
                           "bms" = coef.BMS,
                           "bma.prior" = coef.BMA.prior,
                           "bms.prior" = coef.BMS.prior))
  )
